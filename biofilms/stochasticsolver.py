...
class HillClimber(StochasticSolver):

    def __init__(self, num_params, step_size=1000, decay_rate=0.999, init_range=(0, 300000)):
        super().__init__(num_params=num_params, popsize=1)
        self.step_size = step_size
    ...
    def ask(self):
        if self.best_solution is None:
            self.next_solution = np.random.uniform(low=self.init_range[0], high=self.init_range[1],
                                                   size=(1, self.num_params))
        else:
            self.next_solution = np.array(
                [self.best_solution + np.random.uniform(low=-self.step_size, high=self.step_size,
                                                        size=self.num_params)])
        # self.step_size *= self.decay_rate
        return self.next_solution

    def tell(self, fitness_list):
        if fitness_list[0] >= self.best_fitness:
            self.best_solution = self.next_solution[0]
            self.best_fitness = fitness_list[0]

    def result(self):
        return self.best_solution, self.best_fitness


class RND(StochasticSolver):

    def __init__(self, num_params, popsize=1, init_range=(0, 300000)):
        super().__init__(num_params, popsize)
        self.best_solution = None
        self.best_fitness = float("-inf")
        self.solutions = []
        self.init_range = init_range
    ...
    def ask(self):
        self.solutions = np.random.uniform(low=self.init_range[0], high=self.init_range[1],
                                           size=(self.popsize, self.num_params))
        return self.solutions

    def tell(self, fitness_list):
        idx = np.argmax(fitness_list)
        if fitness_list[idx] >= self.best_fitness:
            self.best_solution = self.solutions[idx]
            self.best_fitness = fitness_list[idx]
...
class DummyFunc(object):
    ...
    def __init__(self):
        self.objectives_dict = ObjectiveDict()

    def create_objectives_dict(self):
        self.objectives_dict.add_objective(name="fitness",
                                           maximize=True,
                                           tag="<>",
                                           best_value=0.0,
                                           worst_value=float("-inf"))
        self.objectives_dict.add_objective(name="age",
                                           maximize=False,
                                           tag="<>",
                                           best_value=0,
                                           worst_value=float("inf"))
...
class GeneticAlgorithm(StochasticSolver):

    def __init__(self, num_params, seed, pop_size, init_range):
        super().__init__(num_params=num_params, popsize=pop_size)
        self.inner = Solver.create_solver(name="ga",
                                          seed=seed,
                                          pop_size=pop_size,
                                          genotype_factory="uniform_float",
                                          solution_mapper="direct",
                                          fitness_func=DummyFunc(),
                                          listener=None,
                                          remap=False,
                                          genetic_operators={"gaussian_mut": 0.2, "uniform_cx": 0.8},
                                          ...
                                          upper=init_range[1],
                                          lower=init_range[0],
                                          genotype_filter="positive")
        self.init_range = init_range
        self.gen = 0

    def ask(self):
        if self.gen != 0:
            for child_genotype in self.inner.build_offspring():
                self.inner.pop.add_individual(genotype=child_genotype)
        return [ind.genotype for ind in self.inner.pop if not ind.evaluated]

    def tell(self, fitness_list):
        for ind, f in zip([ind for ind in self.inner.pop if not ind.evaluated], fitness_list):
            ind.fitness = {"fitness": f}
            ind.evaluated = True
        if self.gen != 0:
            self.inner.trim_population()
        self.gen += 1

    def result(self):
        best = self.inner.get_best()
        return best.genotype, best.fitness["fitness"]


class AFPO(StochasticSolver):

    def __init__(self, num_params, seed, pop_size, init_range):
        super().__init__(num_params=num_params, popsize=pop_size)
        self.inner = Solver.create_solver(name="afpo",
                                          seed=seed,
                                          pop_size=pop_size,
                                          genotype_factory="uniform_float",
                                          solution_mapper="direct",
                                          ...
                                          listener=None,
                                          remap=False,
                                          genetic_operators={"gaussian_mut": 1.0},  # "uniform_cx": 0.8},
                                          offspring_size=pop_size,
                                          tournament_size=5,
                                          mu=0.0,
                                          sigma=5000,
...
class SimpleGA(object):
    ...
    def ask(self):
        ...
        if self.first_iteration:
            self.solutions = np.random.random((self.popsize, self.num_params)) * \
                             (self.init_range[0] - self.init_range[1]) + self.init_range[0]
            return self.solutions
        solutions = []
